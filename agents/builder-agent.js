...
async function claude(prompt, system) {
  ...
  if (process.env.MOONSHOT_API_KEY) {
    try {
      const r = await fetchWithRetry('https://api.moonshot.cn/v1/chat/completions', {
        method: 'POST',
...
5. Free limit: ${spec.free_limit} uses
6. Usage nudge (non-blocking banner): when uses === freeLimit - 1, show sticky banner "1 use left — upgrade for unlimited →" with Pro CTA. Do NOT block the tool yet.
7. Paywall (blocking): after uses >= freeLimit. Show TWO tier options:
   - Pro: $${spec.pricing?.monthly_usd || 9}/mo (primary button, indigo)
   - Max: $${Math.round((spec.pricing?.monthly_usd || 9) * 3)}/mo — 10x output limit + priority (secondary, smaller)
   - Annual toggle: "Pay annually — save 2 months" switches both prices to annual amounts ($${Math.round((spec.pricing?.monthly_usd || 9) * 10)}/yr, $${Math.round((spec.pricing?.monthly_usd || 9) * 3 * 10)}/yr)
   - Stripe Pro link: CONFIG.STRIPE_LINK_PRO placeholder
   - Stripe Annual link: CONFIG.STRIPE_LINK_ANNUAL placeholder
8. Email capture in paywall (+3 free uses on submit) — shown below tier buttons
9. PostHog events: tool_opened, tool_used, usage_nudge_shown, free_limit_hit, paywall_shown, upgrade_tier_viewed, payment_clicked (with prop tier:'pro'|'max'|'annual'), email_captured
10. Dark theme: background #0f172a, surface #1e293b, accent #6366f1
11. Font: Inter from Google Fonts
12. Mobile responsive
13. No TypeScript, no build step, no npm imports
14. Viral attribution in every output: "Generated by ${spec.name} — ${spec.slug}.vercel.app"
15. Cross-sell: fetch https://raw.githubusercontent.com/${GITHUB_USERNAME || 'github_user'}/agent-os/main/public/portfolio.json to show 3 related tools in paywall
...
async function ghGetFileSha(owner, repo, path) {
  try {
    const r = await fetchWithRetry(`https://api.github.com/repos/${owner}/${repo}/contents/${path}`, { headers: GH_HEADERS() });
...
async function pushToGitHub(spec, html, copy) {
  ...
  // Fetch existing SHAs in parallel — required for updates to existing files (GitHub 422 without sha)
  const [htmlSha, vercelSha, readmeSha] = await Promise.all([
    ghGetFileSha(owner, repo, 'index.html'),
    ghGetFileSha(owner, repo, 'vercel.json'),
    ghGetFileSha(owner, repo, 'README.md'),
  ]);

  // Sequential pushes — parallel causes 409 "reference already exists" on empty repos
  await ghPushFile(owner, repo, 'index.html', html, 'feat: initial tool (payment links TBD)', htmlSha);
  await ghPushFile(owner, repo, 'vercel.json', vercelJson, 'chore: vercel config', vercelSha);
  await ghPushFile(owner, repo, 'README.md', readme, 'docs: readme', readmeSha);
}
